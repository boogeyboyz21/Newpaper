    ...
    <form onSubmit={submit} className="space-y-2" data-testid={parentId ? "reply-form" : "comment-form"}>
      {(!user || user === false) && (
        <input
          ...
          className="w-full border border-[var(--line)] px-3 py-2 text-sm bg-transparent outline-none text-ink"
        />
      )}
      <textarea
      ...
      <button
        ...
        disabled={busy}
        className="bg-navy text-white px-4 py-2 text-xs font-bold uppercase tracking-wider disabled:opacity-50"
      >
...
// Flatten the nested tree into a list with depth to avoid recursive JSX components.
...
export default function Comments({ articleId }) {
  ...
  return (
    <section data-testid="comments-section" className="mt-10 pt-8 border-t-2 border-ink">
      <h3 className="font-serif-display text-2xl flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] uppercase tracking-widest text-navy border border-navy px-2 py-0.5 ml-2">Live</span>
      </h3>
        ...
        {flat.map((c) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <span className="font-body font-semibold text-sm">{c.author_name}</span>
              {c.is_guest && <span className="text-[10px] uppercase tracking-wider text-ink-soft border border-[var(--line)] px-1">Guest</span>}
            ...
            <div className="flex items-center gap-4 mt-2">
              <button data-testid="upvote-btn" onClick={() => upvote(c.id)} className="flex items-center gap-1 text-xs text-ink-soft hover:text-navy">
                <ThumbsUp size={13} /> {c.upvotes}
              </button>
              <button data-testid="reply-btn" onClick={() => setReplyTo(replyTo === c.id ? null : c.id)} className="flex items-center gap-1 text-xs text-ink-soft hover:text-navy">
